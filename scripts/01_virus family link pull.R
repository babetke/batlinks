...
write.csv(bats, "/Users/brianabetke/Desktop/batlinks/virus association data/bat-virus species associations.csv", row.names = FALSE)
write.csv(fams, "/Users/brianabetke/Desktop/batlinks/virus association data/bat-virus family associations.csv", row.names = FALSE)

# expand fams grid to all possible combos
fams %>% mutate(association = 1) -> fams

# grid search for all possible combos
all=expand_grid(Host=unique(fams$Host),
                VirusFamily=unique(fams$VirusFamily))
all$fam_links = paste(all$Host,all$VirusFamily, sep="-")

# merge
links <- merge(all, fams[c("fam_links","association")], all.x=T)

#pseudoabs
links$status <- ifelse(is.na(links$association),0,links$association)

# save links
write.csv(links, "/Users/brianabetke/Desktop/batlinks/virus association data/bat-virus family links.csv", row.names = FALSE)
